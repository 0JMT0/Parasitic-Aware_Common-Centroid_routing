"""Utility to visualize and route common-centroid layouts using MST/BFS wiring (TOP ONLY)."""
...
# =========================
# ⭐ Routing pattern 網格、CP Sequence v2 與 pattern 集合（top-only 多半無 O/N）
# =========================
...
def edge_pattern(edge_data: Dict[str, Set[str]]) -> Optional[str]:
    """Return O/N when both plates contribute; top-only 時通常回傳 None。"""
...
def build_cp_sequence_v2(
    ...
    sequence: List[str] = [f"{unit_cap_value:.6e}"]
    for pattern, edge in entries:
        (r1, c1), (r2, c2) = edge
        if r1 == r2:
            orientation = "H"
        elif c1 == c2:
            ...
            # Skip unexpected diagonal entries gracefully
            continue
        sequence.append(f"{pattern}@{orientation}({r1},{c1})-({r2},{c2})")
    return sequence
...
def find_component_infos(
    ...
    不排除中心；同一 comp_id 的所有連通區都會被各自 group 成 component。
    """
    rows, cols = matrix.shape
...
def build_component_bridges(
    ...
    ⭐ 中心元件可參與 MST；如涉及中心則不建立鏡像 bridge。
    """
    if not pairs:
        return []

    primary_infos = [pair.primary for pair in pairs]
    ...
    def add_bridge(key_a: Tuple[Coordinate, ...], key_b: Tuple[Coordinate, ...], path: List[Coordinate]) -> None:
        ...
        # # 涉及中心 (-2) 時，不建立鏡像 bridge
        # if source.comp_id == -2 or target.comp_id == -2:
        #     return
...
def plate_coordinate(points: Iterable[Coordinate], plate: str, rows: int) -> Coordinate:
    ...
    if plate == "top":
        return (-1, avg_col)  # top plate anchor
    if plate == "bottom":
        return (rows, avg_col)
...
def route_layout(
    ...
    """Generate common-centroid routing for TOP plate only (group by comp_id, no center special-casing)."""
    rows, cols = matrix.shape
    ...
    # 全部 pair（含沒有鏡像者）一起分配 mode（overlapped / nonoverlapped / both / random）
    ...
    # 1) 每個 component 內部連線（用 spanning tree）
    for pair, mode in zip(pairs, pair_modes):
        ...
        # mirror（若存在：與 primary 同 mode、且邊為鏡像）
        if pair.mirror is not None:
            mode_by_key[pair.mirror.key] = mode
            mirrored_edges = [mirror_edge(e, rows, cols) for e in primary_edges]
            component_edges[pair.mirror.key] = mirrored_edges

    # 2) 建立所有 components（含無鏡像者）之間的橋接（TOP 層）
    component_bridges = build_component_bridges(pairs, rows, cols)

    # 3) 輸出 TOP plate 路由（intra-components）
    for pair in pairs:
    ...
    if bridge_route_paths:
        routes.append(
            RouteResult(
                component_id=-1,   # 專用 ID：bridge network
                method=method,
                mode="single",
                plate="top",
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Route a common-centroid layout using MST/BFS wiring (TOP ONLY).")
    parser.add_argument(
        "--layout-file",
        type=str,
    ...
    parser.add_argument(
        ...
        default="random",
        help="Routing mode selection for mirrored pairs (top only).",
    )
    parser.add_argument(
        "--unit-cap",
        type=float,
    ...
    # CP Sequence v2 + pattern 統計（top-only 多半無 O/N）
    ...
    if cp_entries:
        ...
        print("No adjacent unit capacitors share both plate connections (top-only run).")
    ...
    if histogram:
        ...
        print("No patterns to report (top-only run).")

    if not args.no_plot:
        if plt is None:
            print("matplotlib is not installed; skipping visualization.")
        else:
            title = f"Routing via {args.method.upper()} (mode={args.mode}) - TOP ONLY"
